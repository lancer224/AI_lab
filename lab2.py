...
#_columns_drop = ['RoomService', 'FoodCourt', 'ShoppingMall', 'Spa', 'VRDeck']
#df = df.drop(columns=_columns_drop, errors='ignore')
X = df.drop(['Age'], axis = 1)
y = df['Age']

X_train, X_test, y_train, y_test = train_test_split(X, y, test_size = 0.4, random_state = 37)

X_test, X_val, y_test, y_val = train_test_split(X_test, y_test, test_size=0.4, random_state=37)
